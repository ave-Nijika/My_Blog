...
import { getArticlesByTagSlug, countArticlesByTagSlug, getAllTagsWithCount } from "@/lib/queries";
...
export async function generateMetadata({
  ...
  return {
    title: `Tag: ${tag.name}`,
    description: `Posts tagged with "${tag.name}" (${tag._count.articles} posts)`,
  };
}
...
export default async function TagPage({
  ...
  const [tag, articles, totalCount] = await Promise.all([
    getAllTagsWithCount().then((tags) => tags.find((t) => t.slug === slug)),
    getArticlesByTagSlug(slug, page, perPage),
    countArticlesByTagSlug(slug),
  ]);
    ...
    <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6">
      <Reveal translateY={10}>
        <nav className="mb-4 flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 max-sm:text-sm">
          <Link href="/tags" className="transition-colors hover:text-[color:rgb(var(--ba-primary))]">
            {t.common.tags}
          </Link>
          <span aria-hidden>/</span>
          <span className="text-[color:rgb(var(--ba-primary))]">#{tag.name}</span>
        </nav>
